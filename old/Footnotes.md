...
- 특이사항
    1. 현재 쓰고 있는 스킨에서는 각주 지원을 하지 않는 것 같다. GFM CSS 덮어쓰기를 하고 나머지는 정상적으로 들어온 것 같은데.. 적용이 안된다.
    2. 이럴 경우, 아래의 포맷으로 각주를 달아주면 정상적용 된다.

- Format
> 문장 중간 각주를 달고 싶은 곳[Footnote]각주 내용[/Footnote]
>> 위 Footnote에서 F만 소문자 f로 바꿔주면 된다.

- Example
이것은 첫번째 각주다[footnote]이것은 첫 각주[/footnote]. 그리고 이것은 두번째 각주다[footnote]이것은 두번째 각주[/footnote].
경우에 따라, 긴 각주를 다는 것도 가능하다[footnote]이것은 세번째 각주이다.<br>각주가 길어져도, 글 하단에 자동으로 추가된다.<br>필요한 경우, 각주 내용 도중 `코드`삽입과 같이 글자 *Formatting*도 가능하다..[/footnote]
